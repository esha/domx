(function(D) {
  ...
  */
    ...
    test('via x-repeat="selector"', function() {
        // gets x-repeat w/same attributes
        var el = D.query('.other');
        equal(el.tagName.toLowerCase(), 'x-repeat', "should already be x-repeat");
        equal(el.getAttribute('x-repeat'), '.foo', "should have x-repeat");
        equal(el.hasAttribute('x-repeat-none'), true, "should have x-repeat-none");
        equal(el.innerHTML, "There are &lt;b&gt;no&lt;/b&gt; foos.", "has same innerHTML");
        equal(el.hasAttribute('x-repeat-id'), true, "should have x-repeat-id");
    ...
    test('easier together than apart', function() {
        ...
        ok(self.hasAttribute('x-repeat'), "should have x-repeat");
        
        // none message in attribute
        equal(self.getAttribute('x-repeat-none'), "No items.", "should have x-repeat-none");
        equal(self.innerHTML, self.getAttribute('x-repeat-none'), "innerHTML should be same as none attr");
        ...
        selves.each(function(el, i) {
            equal(el.getAttribute('x-index'), i, "should have proper index");
        });

        // x-repeat-first
        var first = self.repeat('first');
        ok(self.hasAttribute('x-repeat-first'), "has x-repeat-first");
        equal(first.getAttribute('x-index'), 0, "new repeats should go first");
        ...
        self.repeat([1,2,3]);
        equal(D.queryAll('.self[x-index]').length, 4, "plue 3 more repeats makes 4");
