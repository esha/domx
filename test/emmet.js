(function(D) {
  ...
  */
	...
	test("nested", function() {
		var nested = D.html.body.add('div>div');
		equal(nested.tagName, "DIV", "Tags created.");
		ok(D.html.body.div.only(-1).div, 'Tags found.');
		nested.parentNode.remove();
	});

	test("siblings with kids", function() {
		var h1 = D.html.body.add('section.bro+section.bro').add('h1');
	...
	test("element id", function() {
		ok(D.html.body.add('span#foo'), 'have element');
		equal(D.html.body.span.id, 'foo', 'has right id');
		D.query('#foo').remove();
	});

	test("element class", function() {
		ok(D.html.body.add('div#classes.bar.woogie'), 'have element');
		equal(D.query('#classes').getAttribute('class'), 'bar woogie', 'has right classes');
		D.query('#classes').remove();
	});

	test("element attr", 4, function() {
		ok(D.html.body.add('div#attrs[test foo=bar bar="woogie baz"]'), 'have element');
	...
	test("climb up context", 4, function() {
		equal(D.html.body.add('p>div>div>span^h2^^h1').tagName, 'H1', 'right element');
		ok(D.html.body.p.div.div.span, 'have initial tree');
		ok(D.html.body.p.div.h2, 'h2 went in right place');
		ok(D.html.body.h1, 'h1 went in right place');
		D.html.body.p.remove();
		D.html.body.h1.remove();
	});

	test("multiplier", 4, function() {
		var spans = D.html.body.add('span*5');
	...
	test("text", function() {
		var text = D.html.body.add('p#text{hello world!}');
		ok(text, 'have element');
		equal(D.query('#text').textContent, 'hello world!', 'has right text');
		text.remove();
		var mixed = D.html.body.add('p#mixed{a}>span+{ b}');
